"""Run Bob's gateway on port 7475. Run setup_local_test.py first."""
import asyncio, sys
...
STATE = ROOT / "test-data" / "bob.json"
PASSPHRASE = "bob-dev-pass"

async def main():
    agent = AgentState.load(STATE, PASSPHRASE.encode())
    config = GatewayConfig(host="127.0.0.1", port=7475, poll_interval=3.0)
    print(f"Bob gateway running on ws://127.0.0.1:7475")
