// Cross-gateway DM REACTION smoke — proves DM secondary ops federate.
//
// Runs the full two-gateway handshake (Alice@A + Bob@B: discovery → friend
// request → accept → cert exchange → E2E DM), then Bob reacts 🔥 to Alice's
// message and we assert BOTH Bob's local echo AND Alice receiving the reaction
// cross-gateway. This is the ground-truth guard on the federation IDENTITY MODEL:
// the reaction relay's from_webid MUST be the sender's GATEWAY did (not the
// browser session did) or the receiver's get_relationship_by_did misses and the
// reaction is silently dropped. Regression guard for the R54 identity-model fix.
//
//   node web/smoke_dm_reaction.mjs    (spawns both gateways; needs python + chrome)
//
// Exit 0 = Bob's cross-gateway 🔥 reaction reached Alice.
...
const MSG = 'dmreact-' + Math.random().toString(36).slice(2, 7);
...
try {
  ...
  // ── THE REACTION ROUND-TRIP: Bob reacts to Alice's message; Alice must see it.
  // This is the ground-truth check on the federation IDENTITY MODEL: the reaction
  // relay carries from_webid, and Alice's gateway only delivers/attributes it if
  // from_webid matches her relationship with Bob. If from_webid is the browser
  // client did but relationships are keyed by the gateway did, this FAILS.
  step = 'bob-find-msg-id';
  const mid = await bob.evaluate((t) => {
    for (const b of document.querySelectorAll('[data-msg-action="react"]')) {
      const wrap = document.getElementById('msg-' + b.dataset.msgId);
      if (wrap && wrap.textContent.includes(t)) return b.dataset.msgId;
    }
    return null;
  }, MSG);
  if (!mid) { fail('could not find Alice\'s message id on Bob to react to'); throw new Error('stop'); }
  console.log(`  · reacting to message ${mid.slice(0, 12)}…`);

  step = 'bob-react';
  await bob.evaluate((id) => document.querySelector(`[data-msg-action="react"][data-msg-id="${id}"]`)?.click(), mid);
  await sleep(300);
  const pickerShown = await bob.evaluate(() => { const p = document.getElementById('emoji-picker'); return p ? getComputedStyle(p).display : 'no-picker'; });
  await bob.evaluate(() => document.querySelector('#emoji-picker span[data-emoji="🔥"]')?.click());
  await sleep(500);

  // Bob should see his own reaction immediately (local echo).
  step = 'bob-sees-own-reaction';
  await bob.waitForFunction((id) => document.getElementById('reactions-' + id)?.textContent.includes('🔥'),
    { timeout: 5000 }, mid).catch(async () => {
      fail('Bob did not see his OWN reaction (send failed)');
      console.error(`  · picker display after react-click: "${pickerShown}"`);
      const bc = (bob._console || []).filter(l => /react|error|participant|not a|warn/i.test(l)).slice(-8);
      console.error(`  · Bob console:\n${bc.map(l => '    ' + l).join('\n') || '    (none)'}`);
      const gb = (procs[1]._log() || '').split('\n').filter(l => /react|participant|Not a|Unknown/i.test(l)).slice(-8);
      console.error(`  · Bob GW:\n${gb.map(l => '    ' + l).join('\n') || '    (none)'}`);
    });
  if (process.exitCode) throw new Error('stop');

  step = 'alice-sees-reaction';
  await alice.waitForFunction((id) => document.getElementById('reactions-' + id)?.textContent.includes('🔥'),
    { timeout: 15000 }, mid).catch(async () => {
      const cont = await alice.evaluate((id) => document.getElementById('reactions-' + id)?.textContent ?? '(no reactions container)', mid);
      fail(`Alice did NOT receive Bob's cross-gateway reaction — reactions container: "${cont}"`);
      const rx = /reaction|relationship|dm_reaction|from_webid|ignored|received/i;
      const pick = (log) => (log || '').split('\n').filter(l => rx.test(l)).slice(-8).map(l => '    ' + l).join('\n') || '    (none)';
      console.error(`  · Alice GW (reaction relay):\n${pick(procs[0]._log())}`);
      console.error(`  · Bob GW (reaction relay):\n${pick(procs[1]._log())}`);
    });

  step = 'done';
  if (!process.exitCode) {
    console.log('  ✓ cross-gateway DM reaction OK — Bob reacted 🔥 to Alice\'s message and');
    console.log('    Alice received it (federation identity model + dm_reaction relay verified).');
  }
} catch (e) {
