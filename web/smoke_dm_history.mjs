// E2E DM history smoke — proves DMs stay readable after reload/reopen.
//
// DMs are E2E (forward Double Ratchet), so the server holds only ciphertext the
// ratchet can't re-decrypt; without the local plaintext cache (dmhistory.js), a
// reopened E2E DM shows "[could not decrypt]". Reuses the two-gateway federation
// flow (Alice@A + Bob@B, real cross-gateway handshake + E2E DM), then RELOADS Bob
// and reopens the DM and asserts the message is still plaintext.
//
//   node web/smoke_dm_history.mjs    (spawns both gateways; needs python + chrome)
//
// Exit 0 = the E2E DM was still readable plaintext after Bob reloaded + reopened.
...
const MSG = 'dmhist-' + Math.random().toString(36).slice(2, 7);
...
try {
  ...
  // ── THE ROUND'S ASSERTION: reload Bob and reopen the DM. The message is E2E —
  // the server only holds ciphertext the forward ratchet can't re-decrypt — so
  // without the local plaintext cache (dmhistory.js) it would come back as
  // "[could not decrypt]". It must render as readable plaintext.
  step = 'reload-bob';
  await bob.reload({ waitUntil: 'load', timeout: 20000 });
  await bob.evaluate(() => { const m = document.getElementById('onboarding-modal'); if (m) m.style.display = 'none'; });
  await bob.waitForFunction(() => document.querySelector('.dot')?.classList.contains('online'), { timeout: 15000 })
    .catch(() => fail('Bob did not reconnect after reload'));
  if (process.exitCode) throw new Error('stop');

  step = 'bob-reopen-dm';
  await bob.waitForFunction(() => document.querySelector('#contacts-list li, #dm-list li'), { timeout: 15000 })
    .catch(() => fail('DM/contact gone from Bob after reload'));
  if (process.exitCode) throw new Error('stop');
  await sleep(500);
  await bob.evaluate(() => (document.querySelector('#dm-list li, #contacts-list li')).click());
  await bob.waitForFunction(() => {
    const h = document.getElementById('chat-header-name'); return h && !/welcome/i.test(h.textContent || '');
  }, { timeout: 8000 }).catch(() => fail('Bob could not reopen the DM after reload'));
  if (process.exitCode) throw new Error('stop');
  await sleep(1500); // let server history + local plaintext merge settle

  step = 'assert-plaintext-after-reload';
  const feedAfter = await bob.evaluate(() => document.getElementById('message-feed')?.textContent || '');
  if (!feedAfter.includes(MSG)) {
    // Diagnostics: what's in the local DM-history store + the reopened thread id?
    const diag = await bob.evaluate(async () => {
      ...
      return {
        activeThread: (window.__activeThreadId || null),
        idbCount: rows.length,
        idb: rows.map(r => ({ id: (r.message_id || '').slice(0, 10), thread: (r.thread_id || '').slice(0, 18), content: (r.content || '').slice(0, 20) })),
      };
    });
    console.error(`  · idb dm-history records: ${diag.idbCount}`);
    console.error(`  · ${JSON.stringify(diag.idb)}`);
    console.error(`  · feed tail: "${feedAfter.slice(-120)}"`);
    fail(/could not decrypt|decryption error/i.test(feedAfter)
      ? `after reload the E2E DM shows "[could not decrypt]" — plaintext history NOT persisted`
      : `after reload the DM message "${MSG}" is missing from history`);
  }

  step = 'done';
  if (!process.exitCode) {
    console.log('  ✓ E2E DM history OK — cross-gateway E2E DM delivered + decrypted on Bob,');
    console.log(`    AND still readable plaintext "${MSG}" after Bob reloaded + reopened the DM.`);
  }
} catch (e) {
