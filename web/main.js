...
import { podWriteMessageWithIndex, podWriteRoomMeta, podReadMessages, podSetContainerAcl,
        ...
        function handleEvent(event) {
            switch (event.type) {
                ...
                case "file_offer":    handleFileOffer(event); break;
                case "file_accept":   handleFileAccept(event); break;
                case "file_reject":   handleFileReject(event); break;
                case "file_chunk":    handleFileChunk(event); break;
                case "file_complete": handleFileComplete(event); break;
                case "file_unreachable": handleFileUnreachable(event); break;
        ...
        // ── R39: Chunked large-file transfer (DMs, real-time, recipient online) ──
        ...
        async function sendFileChunked(file, toWebid) {
            ...
            for (let seq = 0; seq < total; seq++) {
                ...
                sendCmd("file_chunk", { to_webid: toWebid, file_id: fileId, seq, data: _u8ToB64(slice) });
        ...
        function handleFileOffer(event) {
            if (event.size_bytes > 25 * 1024 * 1024) {
        ...
        function handleFileComplete(event) {
            ...
            let totalLen = 0;
            const parts = [];
            for (let i = 0; i < rec.total; i++) { const u8 = _b64ToU8(rec.chunks[i] || ""); parts.push(u8); totalLen += u8.length; }
            const full = new Uint8Array(totalLen);
            let off = 0;
            for (const p of parts) { full.set(p, off); off += p.length; }
            const dataB64 = _u8ToB64(full);
            const fromWebid = rec.fromWebid;
            delete _incomingFiles[event.file_id];
            const threadId = (activeView && activeView.peerWebid === fromWebid) ? activeView.id : fromWebid;
            renderMessage({
        ...
        document.getElementById("file-input").onchange = (e) => {
            ...
            if (file.size > 524288) {
                ...
                sendFileChunked(file, peerWebid).catch(err => {
