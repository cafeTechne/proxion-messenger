...
class MiscHandlerMixin:
    ...
    async def _handle_block(self, websocket, data: dict) -> None:
        ...
        if not webid:
            return
        canonical = self._canonical_block_id(webid)   # R90 B: one identity form
        owner = self._client_webids.get(websocket, "")
        # Per-owner now that the receive path enforces per-owner (R90 B). The global union
        # file is no longer mirrored (that was the cross-owner leak); it is written only
        # when there is no store, so a store-less gateway still enforces something.
        if owner and self._store:
            self._store.set_block(owner, canonical, True)
        else:
            self.blocklist.block(canonical)
    ...
    async def _handle_unblock(self, websocket, data: dict) -> None:
        ...
        if not webid:
            return
        canonical = self._canonical_block_id(webid)
        owner = self._client_webids.get(websocket, "")
        if owner and self._store:
            self._store.set_block(owner, canonical, False)
        # Also clear the legacy union file if no owner still blocks them, so unblocking a
        # pre-R90 (owner-less) block actually takes effect. Cover both stored forms.
        if not (self._store and self._store.is_blocked_by_anyone(canonical)):
            self.blocklist.unblock(canonical)
